...
export const Header: React.FC<HeaderProps> = ({ currentLanguage, onLanguageChange }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${
      isVisible 
        ? 'transform translate-y-0 opacity-100 backdrop-blur-xl' 
        : 'transform -translate-y-full opacity-0 backdrop-blur-none'
    } tesla-nav-enhanced`}>
      <div className="tesla-container">
        <div className="flex items-center justify-between h-16 relative">
          ...
          <div className="flex items-center cursor-pointer" onClick={handleLogoClick}>
            ...
            <div className="ml-3 flex flex-col justify-center h-6 hidden sm:flex transition-all duration-300 hover:scale-105">
            ...
            <div className="ml-3 sm:ml-4 md:ml-6">
              {/* Mobile version */}
              <div className="sm:hidden text-black font-inter font-bold text-xs tracking-wider uppercase leading-tight transition-all duration-300 hover:scale-105">
                <div>AL MARJAN</div>
                <div>ISLAND</div>
              </div>
              {/* Desktop version */}
              <div className="hidden sm:block px-4 py-2 bg-black text-white rounded-full text-xs font-medium tracking-wider transition-all duration-300 hover:bg-gray-800 hover:scale-105 shadow-lg">
